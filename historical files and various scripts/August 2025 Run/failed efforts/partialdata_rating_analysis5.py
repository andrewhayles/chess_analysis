...
def objective_function_weights(weights_array, df, engine_ratings):
    ...
    # **MODIFICATION**: Exclude Maia engines from the curve fitting process for weight optimization
...
# **MODIFICATION**: Create the final fit_df for analysis using only Stockfish engines
fit_df = summary_df.dropna(subset=['rating'])
fit_df = fit_df[~fit_df['engine_name'].str.contains('maia', case=False)]
...
def objective_function_ratings(adjustable_ratings, adjustable_hits):
    """Objective function for R^2 calculation with only adjustable ratings."""
    try:
        p0 = [max(adjustable_hits), 0.005, np.median(adjustable_ratings)]
        bounds = ([min(adjustable_hits)-1, 0, min(adjustable_ratings)-1], [max(adjustable_hits)*1.1, 0.1, max(adjustable_ratings)*1.1])
        popt, _ = curve_fit(logistic_function, adjustable_ratings, adjustable_hits, p0=p0, bounds=bounds, maxfev=10000)
        residuals = adjustable_hits - logistic_function(adjustable_ratings, *popt)
        ss_res = np.sum(residuals**2)
        ss_tot = np.sum((adjustable_hits - np.mean(adjustable_hits))**2)
...
print("--- Logistic Regression Analysis (Stockfish engines only) ---")
if len(x_data) > 2:
    try:
        p0 = [max(y_data), 0.005, np.median(x_data)]
        bounds = ([min(y_data)-1, 0, min(x_data)-1], [max(y_data)*1.1, 0.1, max(x_data)*1.1])
        ...
        r_squared_initial, initial_rating = plot_results(x_data, y_data, popt, player_hit_percentage,
                                                         'Initial Hit Count % vs. Stockfish Rating',
    ...
    print("Not enough Stockfish engines to perform logistic fit.")
...
if r_squared_initial < 0.9 and len(x_data) > 0:
    ...
    initial_stockfish_ratings = fit_df['rating'].values
    stockfish_hits = fit_df['hit_percentage'].values
    
    ORACLE_RATING_CAP = 3650
    rating_bounds = [(r * 0.5, min(r * 1.5, ORACLE_RATING_CAP)) for r in initial_stockfish_ratings]

    result = minimize(objective_function_ratings, initial_stockfish_ratings, 
                      args=(stockfish_hits,), 
                      method='L-BFGS-B', bounds=rating_bounds)
    adjusted_sf_ratings = result.x

    adjusted_map = dict(zip(fit_df['engine_name'], adjusted_sf_ratings))
    ...
    try:
        x_data_adj = final_fit_df[final_fit_df['engine_name'].str.contains('stockfish', case=False)]['adjusted_rating'].values
        y_data_adj = final_fit_df[final_fit_df['engine_name'].str.contains('stockfish', case=False)]['hit_percentage'].values
        ...
        r_squared_adj, final_rating = plot_results(x_data_adj, y_data_adj, popt_adj, player_hit_percentage,
                                                   'Adjusted Hit Count % vs. Adjusted Stockfish Rating',
