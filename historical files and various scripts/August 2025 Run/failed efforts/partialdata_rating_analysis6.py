...
# --- 3. Set Weights and Calculate Hit Percentages ---

# **MODIFICATION**: Using fixed, logical weights instead of optimizing them.
# This provides a more stable and interpretable hit percentage metric.
fixed_weights = {0: 1.0, 1: 0.5, 2: 0.25}
print("--- Using Fixed Weights for Analysis ---")
print(f"Weights: {fixed_weights}\n")

# Recalculate with fixed weights for ALL engines for reporting
engine_hit_percentages = calculate_hit_percentage(df, fixed_weights)
...
print("--- Injecting Hypothetical Data Points for Test ---")
hypothetical_data = pd.DataFrame([
    {'engine_name': 'dragon', 'rating': 3600, 'hit_percentage': 90.0},
    {'engine_name': 'stockfish_full', 'rating': 3650, 'hit_percentage': 95.0}
])
...
print("--- Hit Percentage Analysis with Fixed Weights ---")
...
if r_squared_initial < 0.9 and len(x_data) > 0:
    ...
    if not adjustable_df.empty:
        ...
        # **MODIFICATION**: Display all non-Maia engines in the final adjusted table
...
try:
    ...
    with open('rating_analysis_results.txt', 'w') as f:
        ...
        f.write("="*50 + "\n\n")
        f.write("Fixed Weights Used for Analysis:\n")
        f.write(f"  - Top Move (Rank 0): {fixed_weights[0]:.4f}\n")
        f.write(f"  - 2nd Best (Rank 1): {fixed_weights[1]:.4f}\n")
        f.write(f"  - 3rd Best (Rank 2): {fixed_weights[2]:.4f}\n\n")
