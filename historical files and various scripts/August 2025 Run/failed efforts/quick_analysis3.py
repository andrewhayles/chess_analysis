...
def analyze_and_graph():
    print("--- Live Analysis Visualizer (Logistic, Weighted) ---")
    ...
    # Count how many positions each engine was tested on
    counts_df = log_df.groupby('engine_name')['score'].count().reset_index(name='n_positions')

    # Merge averages and counts
    avg_scores_df = pd.merge(avg_scores_df, counts_df, on='engine_name')

    # Separate player vs engines
    ...
    try:
        ...
        n_positions = final_df['n_positions'].values

        # Weight: more positions = higher weight (lower sigma)
        sigma = 1 / np.sqrt(n_positions)

        p0 = [1.0, 0.005, 1800]  # initial guesses
        popt, _ = curve_fit(
            logistic,
            xdata,
            ydata,
        ...
        print(f"\n--- Current Overall Status (Logistic, Weighted) ---")
        ...
        plt.plot(x_vals, y_vals, 'r--', label='Weighted Logistic Fit')

        # Scatter engine points (scaled by number of positions)
        plt.scatter(
            final_df['rating'],
            final_df['average_hit_score'],
            s=30 + final_df['n_positions'] / 5,  # bubble size ~ sample size
            alpha=0.8
        )
        ...
        plt.title('Hit Score vs. Engine Rating (Weighted Logistic)', fontsize=16)
