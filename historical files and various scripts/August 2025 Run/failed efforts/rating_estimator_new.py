...
import os
import json
import random  # --- IMPORTED FOR RANDOM SAMPLING ---
...
def get_positions_from_pgn(pgn_path, player_name, positions_per_game, start_move):
    ...
    --- MODIFIED TO RANDOMLY SAMPLE POSITIONS FROM EACH GAME ---
    """
    ...
    with open(pgn_path, 'r', errors='ignore') as pgn:
        while True:
            try:
                ...
                # --- Collect all eligible positions from the current game ---
                ...
                # --- Randomly sample from the collected positions ---
                if positions_per_game is not None and len(eligible_positions_in_game) > 0:
                    if len(eligible_positions_in_game) > positions_per_game:
                        # If more positions are available than needed, sample randomly
                        ...
                        # If fewer or equal positions are available, take all of them
                        all_selected_positions.extend(eligible_positions_in_game)
                elif positions_per_game is None:
                    # If no limit is set, take all eligible positions
                    all_selected_positions.extend(eligible_positions_in_game)
...
def main():
    ...
    if not items_to_analyze:
        ...
        for item in items_to_analyze:
            ...
            # --- Append new row to in-memory DataFrame efficiently ---
