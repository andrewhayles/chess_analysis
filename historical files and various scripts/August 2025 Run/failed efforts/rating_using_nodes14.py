...
engine_configs = {
    ...
    'dragon': {
        'path': 'C:/Users/desja/Documents/my_programming/chess_analysis/engines/dragon/dragon_05e2a7/Windows/dragon-64bit.exe',
        'protocol': 'uci',
        # MODIFIED: Reduced Hash size to prevent memory errors
        'options': {'Hash': 1024, 'Threads': 16},
        'search_param': 'nodes',
        'search_value': 500000
    },
    'stockfish_full_1': {
        'path': 'C:/Users/desja/Documents/my_programming/chess_analysis/engines/stockfish/stockfish-windows-x86-64-sse41-popcnt.exe',
        'protocol': 'uci',
        # MODIFIED: Reduced Hash size to prevent memory errors
        'options': {'Hash': 1024, 'Threads': 16},
        'search_param': 'nodes',
        'search_value': 5000000
    }
}
# --- Utility Functions ---
...
def get_simple_fen(board):
    return " ".join(board.fen().split(" ")[:4])

def get_oracle_analysis(board, oracle_engine, cache):
    simple_fen = get_simple_fen(board)
    if simple_fen in cache:
        return cache[simple_fen]

    print(f"  --> FEN not in cache. Analyzing new position: {board.fen()}")
...
def main():
    ...
    try:
        ...
        # MODIFIED: Set a reasonable hash size for the oracle engine as well
    ...
    for name, config in engine_configs.items():
        try:
            engine = chess.engine.SimpleEngine.popen_uci(config['path'], timeout=30)
