...
LOG_BUFFER_SIZE = 75  # how many rows to buffer before writing to disk
...
def main():
    ...
    if os.path.exists(GRANULAR_LOG_PATH):
        try:
            prev_df = pd.read_csv(GRANULAR_LOG_PATH)
            for _, r in prev_df.iterrows():
                existing_results.add((r['fen'], r['engine_name']))
            log_file = open(GRANULAR_LOG_PATH, "a")
            print(f"Resuming from {len(existing_results)} previously saved results.")
        except Exception:
            log_file = open(GRANULAR_LOG_PATH, "w")
            log_file.write("fen,engine_name,score\n")
    else:
        log_file = open(GRANULAR_LOG_PATH, "w")
        log_file.write("fen,engine_name,score\n")
    ...
    for _, row in benchmark_engine_info.iterrows():
        ...
        if engine:
            # Maia can't handle multi-threading well
            if "maia" in engine_name.lower():
                engine.configure({"Threads": 1})
            else:
                engine.configure({"Threads": ENGINE_THREADS})
            benchmark_engines[engine_name] = engine
    ...
    with tqdm(total=pbar_total, desc="Analyzing Positions") as pbar:
        for fen in fens_for_analysis:
            ...
            for _, row in benchmark_engine_info.iterrows():
                ...
                if (fen, name) in existing_results:
                    pbar.update(1)
                    continue
