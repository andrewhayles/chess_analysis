...
def run_engine_evaluations(oracle_cache):
    ...
    for _, engine_row in engines_df.iterrows():
        ...
        try:
            # Handle multiple limit types (Nodes, Depth, etc.)
            ...
            # *** FIX: If no limit is specified, use a default time limit. ***
            # This is useful for engines like Maia that don't rely on deep search.
            if not limit_args:
                limit_args["time"] = 0.05 # Default to 0.05 seconds
            ...
            with chess.engine.SimpleEngine.popen_uci(engine_path) as engine:
                ...
                for fen, oracle_best_value in tqdm(oracle_cache.items(), desc=f"Analyzing {engine_name}"):
                    ...
                    # Handle old cache format where the whole PV list might be stored
