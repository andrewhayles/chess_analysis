...
class MasterAnalyzer:
            ...
            with open(pgn_path) as pgn:
                while True:
                    ...
                    for i, move in enumerate(game.mainline_moves()):
                        board.push(move)
                        # We want positions after the opening phase
                        if (i + 1) >= self.args.skip_moves: 
    ...
    def run_ladder_collection(self):
        ...
        # *** NEW: Sanity check the opening suite PGN before starting ***
        ...
        if not opening_positions:
            raise ValueError(f"No usable positions found in the opening suite PGN: '{self.args.opening_suite}'. "
                             f"Please ensure the file exists, is not empty, and contains valid games. "
                             f"Positions are taken after move {self.args.skip_moves // 2}.")
    ...
    def run_elo_estimation(self):
        ...
        try:
            for board in pbar:
                ...
                # The move to evaluate is the last move played to reach this board state
