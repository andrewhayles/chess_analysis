...
def calculate_cai_for_move(oracle_engine, board, test_engine_move, oracle_best_move_analysis, constants, oracle_limit):
    ...
    # Use the provided oracle_limit for consistent evaluation.
...
def analyze_position_worker(job):
    ...
    try:
        ...
        # Create separate Limit objects for the Oracle and Test engines
...
def optimize_and_compare():
    ...
    try:
        # FIXED: Added comment='#' to ignore lines starting with # in the CSV.
    ...
    # Package limits to be passed to worker processes
    ...
    for i, constants in enumerate(constant_combinations):
        ...
        for fen in test_suite_fens:
            for name in engines_to_test:
                ...
                jobs.append((fen, name, executable, oracle_executable_name, constants, limits))
