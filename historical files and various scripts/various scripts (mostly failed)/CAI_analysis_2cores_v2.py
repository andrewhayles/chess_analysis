...
# The script will automatically identify the highest-rated engine as the Oracle.
# You can override it here if you want (e.g., ORACLE_ENGINE_NAME = "stockfish_15.exe").
...
def analyze_position_worker(job):
    """Worker function that now returns a status tuple for better error handling."""
    fen, test_engine_name, oracle_engine_name, constants = job
    oracle_engine, test_engine = None, None
    try:
        oracle_path = os.path.join(ENGINE_DIR, oracle_engine_name)
        test_path = os.path.join(ENGINE_DIR, test_engine_name)
        ...
        # Return a descriptive error instead of failing silently.
        error_msg = str(e).replace('\\\\', '\\') # Clean up path formatting for display
...
def optimize_and_compare():
    ...
    try:
        ...
        engine_data = {row['engine_name']: {'rating': row['rating']} for _, row in df.iterrows()}
    except Exception as e:
    ...
    print(f"Oracle Engine: {oracle_name} (Rating: {engine_data.get(oracle_name, {}).get('rating', 'N/A')})")
    ...
    for i, constants in enumerate(constant_combinations):
        ...
        jobs = [(fen, name, oracle_name, constants) for fen in test_suite_fens for name in engines_to_test]
        ...
        # NEW: Separate successes and failures for clear reporting
