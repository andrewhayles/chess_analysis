...
# It can also contain an optional 'executable_name' column. If present,
...
def calculate_cai_for_move(oracle_engine, board, test_engine_move, oracle_best_move_analysis, constants):
    ...
    if len(oracle_best_move_analysis) < 2:
        ...
        # Get score from the current player's point of view first.
        pov_score1 = oracle_best_move_analysis[0]['score'].pov(board.turn)
        pov_score2 = oracle_best_move_analysis[1]['score'].pov(board.turn)
        
        # Now, safely get the centipawn value, converting mates to a large number.
...
def analyze_position_worker(job):
    """Worker function that now returns detailed error information."""
    fen, test_engine_name, test_engine_executable, oracle_executable, constants = job
    oracle_engine, test_engine = None, None
    oracle_path, test_path = None, None # Define here for use in the except block
    try:
        ...
        # NEW: Return the exact path that was attempted, for better debugging.
...
def optimize_and_compare():
    ...
    for i, constants in enumerate(constant_combinations):
        ...
        if failed_analyses:
            ...
            # NEW: Unpack the path from the error result for detailed logging.
