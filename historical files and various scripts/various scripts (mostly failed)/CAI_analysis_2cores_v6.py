...
def analyze_position_worker(job):
    ...
    try:
        ...
        # NEW: Suppress engine startup messages (like the LC0 banner)
        # by redirecting stderr to DEVNULL.
        ...
        # NEW: Robustly quit engines, ignoring errors if they already crashed.
        if oracle_engine:
            try:
                oracle_engine.quit()
            except chess.engine.EngineTerminatedError:
                pass # Engine already terminated, which is fine.
        if test_engine:
            try:
                test_engine.quit()
            except chess.engine.EngineTerminatedError:
