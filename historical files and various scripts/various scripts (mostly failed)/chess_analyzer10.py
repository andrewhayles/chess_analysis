...
# games in parallel and provides a live-updating log file.
...
def build_model_with_uci_levels(engine, move_templates_for_model):
    ...
    for elo in elo_levels:
        # This log message will now appear in the console from the worker process
...
def process_game(game_tuple):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(ENGINE_PATH) as engine:
            ...
            # Step 2: Filter templates for only the target player's positions.
            player_color = chess.WHITE if game.headers.get("White") == PLAYER_TO_ANALYZE else chess.BLACK
            player_specific_templates = {}
            board = game.board()
            for move in game.mainline_moves():
                if board.turn == player_color:
                    # Use .get() to avoid errors if a FEN is somehow missing
                    player_specific_templates[board.fen()] = move_templates.get(board.fen(), set())
                board.push(move)

            # Step 3: Build the performance model using the player-specific positions.
            ...
            # Step 4: Plot the model for this game.
            ...
            # Step 5: Analyze human players' hit rates against the full template.
            player_results = analyze_player_hits(game, move_templates)
            
            # Step 6: Package results to be returned to the main process.
            game_data = {
                'game_num': game_num,
