...
ANALYSIS_TIME_LIMIT = 6.0  # Time for oracle engine to find the best move and its score.
...
def analyze_player_quality(game, move_analyses, engine):
    """Calculates the move quality score for each player in a game."""
    player_qualities = {}
...
def log_move_by_move_analysis(game_num, game, move_analyses, engine):
    """Creates a detailed text log with centipawn loss for each move."""
    ...
    try:
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            for i, move in enumerate(game.mainline_moves()):
                ...
                best_move_analysis = move_analyses.get(fen_before_move)
                
                f.write(f"Move: {move_num_str} {board.san(move)} ({player_name})\n")

                if best_move_analysis:
                    best_move_san = board.san(best_move_analysis['move'])
                    f.write(f"  - Best Move: {best_move_san} (Eval: {best_move_analysis['score']/100.0:.2f})\n")
                    ...
                    if info:
                        ...
                        best_score_pov = best_move_analysis['score'] if board.turn == chess.WHITE else -best_move_analysis['score']
                        centipawn_loss = max(0, best_score_pov - current_player_pov_score)
                        f.write(f"  - Centipawn Loss: {centipawn_loss}\n\n")
                else:
...
def process_game(game_tuple):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(ENGINE_PATH) as engine:
            ...
            log_move_by_move_analysis(game_num, game, move_analyses, engine)
            ...
            player_results = analyze_player_quality(game, move_analyses, engine)
