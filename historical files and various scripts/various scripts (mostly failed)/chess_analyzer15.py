...
#   4. Includes ROBUSTNESS checks to prevent crashes from unexpected engine output.
...
def get_move_analysis(board, engine):
    ...
    try:
        ...
        if not info or not isinstance(info, list) or len(info) == 0:
            logging.warning(f"get_move_analysis received invalid info from engine: {repr(info)}")
            return None
        if 'pv' not in info[0] or 'score' not in info[0]:
            logging.warning(f"get_move_analysis received info dict without pv or score: {repr(info[0])}")
            return None
        ...
        try:
            ...
            for i, (fen, best_move_analysis) in enumerate(move_analyses_for_model.items()):
                ...
                if not result_info or not isinstance(result_info, list) or len(result_info) == 0:
                    logging.warning(f"build_performance_models received invalid info from engine: {repr(result_info)}")
                    continue
                if 'score' not in result_info[0]:
                    logging.warning(f"build_performance_models received info dict without score: {repr(result_info[0])}")
                    continue
...
def analyze_player_quality_and_get_details(game, move_analyses, engine):
    ...
    for move in game.mainline_moves():
        ...
        if best_move_analysis:
            ...
            # *** ROBUSTNESS CHECK ***
            if info and isinstance(info, list) and len(info) > 0 and 'score' in info[0]:
                ...
                logging.warning(f"analyze_player_quality received invalid info: {repr(info)}")
