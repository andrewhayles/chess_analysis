...
#   1. It calculates a MOVE QUALITY score (a float from 0 to 1) based on centipawn loss.
#   2. It uses a HYBRID MODELING approach, automatically selecting the best fit from
#      Linear, Polynomial, and a ROBUST (Tukey's Biweight) regression model.
#   3. It generates detailed MOVE-BY-MOVE logs for each game showing centipawn loss.
#   4. Includes ROBUSTNESS checks to prevent crashes from unexpected engine output by
#      normalizing the engine's response type.
...
# --- NEW MOVE QUALITY CONFIGURATION ---
# Defines the centipawn loss for a move to be considered "perfect" (quality = 1.0)
PERFECT_MOVE_THRESHOLD_CP = 10
# Defines the centipawn loss for a move to be considered a "blunder" (quality = 0.0)
...
def _normalize_info(info):
    """
    Handles the chess engine returning a single dict for multipv=1,
    while the rest of the code expects a list.
    """
...
def get_move_analysis(board, engine):
    ...
    """
    try:
        raw_info = engine.analyse(board, chess.engine.Limit(time=ANALYSIS_TIME_LIMIT), multipv=1)
        info = _normalize_info(raw_info) # Normalize the data structure

        if not info or len(info) == 0 or 'pv' not in info[0] or 'score' not in info[0]:
            logging.warning(f"get_move_analysis received invalid info from engine: {repr(raw_info)}")
            return None
        ...
        logging.error(f"An unexpected error occurred in get_move_analysis: {repr(e)}")
        return None

def calculate_move_quality(centipawn_loss):
    """
    Converts centipawn loss to a quality score between 0 and 1.
    This acts as a "soft" or "float-based" hit count.
    """
    if centipawn_loss <= PERFECT_MOVE_THRESHOLD_CP:
        return 1.0
    if centipawn_loss >= BLUNDER_THRESHOLD_CP:
        return 0.0
    
    # Linearly scale the quality for moves between perfect and blunder
    return 1.0 - ((centipawn_loss - PERFECT_MOVE_THRESHOLD_CP) / (BLUNDER_THRESHOLD_CP - PERFECT_MOVE_THRESHOLD_CP))

def build_performance_models(engine, move_analyses_for_model):
    """
    Generates performance data using the new move quality metric and creates three
    ...
    for elo in elo_levels:
        ...
        try:
            ...
            for i, (fen, best_move_analysis) in enumerate(move_analyses_for_model.items()):
                ...
                raw_result_info = engine.analyse(board, chess.engine.Limit(time=MODEL_BUILD_TIME_LIMIT), multipv=1)
                result_info = _normalize_info(raw_result_info) # Normalize

                if not result_info or len(result_info) == 0 or 'score' not in result_info[0]:
                    logging.warning(f"build_performance_models received invalid info from engine: {repr(raw_result_info)}")
                    continue
                ...
                total_quality_score += calculate_move_quality(centipawn_loss)
                total_moves += 1

            avg_quality = (total_quality_score / total_moves) if total_moves > 0 else 0
    ...
    linear_model.fit(ratings, quality_scores)
    models['Linear'] = (linear_model, linear_model.score(ratings, quality_scores))

    poly_model = make_pipeline(PolynomialFeatures(degree=POLYNOMIAL_DEGREE), LinearRegression())
    poly_model.fit(ratings, quality_scores)
    models['Polynomial'] = (poly_model, poly_model.score(ratings, quality_scores))
    
    X = sm.add_constant(ratings)
    robust_model = sm.RLM(quality_scores, X, M=sm.robust.norms.TukeyBiweight()).fit()
    ss_total = np.sum((quality_scores - np.mean(quality_scores))**2)
    ss_resid = np.sum(robust_model.resid**2)
    robust_r_squared = 1 - (ss_resid / ss_total) if ss_total > 0 else 0
    ...
    final_mse = mean_squared_error(quality_scores, predictions)
    ...
    return final_model, final_r_squared, final_mse, ratings, quality_scores, best_model_type

def analyze_player_quality_and_get_details(game, move_analyses, engine):
    """
    Calculates player quality scores and also returns a detailed list of
    move data for logging purposes. This is the main analysis function for human players.
    """
    player_qualities = {}
    white_player = game.headers.get("White", "White")
    black_player = game.headers.get("Black", "Black")
    player_qualities[white_player] = {'total_quality': 0, 'moves': 0}
    player_qualities[black_player] = {'total_quality': 0, 'moves': 0}
    
    move_details_for_log = []
    ...
    for move in game.mainline_moves():
        ...
        if best_move_analysis:
            detail['best_move_san'] = board.san(best_move_analysis['move'])
            detail['best_move_eval'] = best_move_analysis['score']
            player_name = detail['player']
            
            board.push(move)
            raw_info = engine.analyse(board, chess.engine.Limit(depth=10))
            info = _normalize_info(raw_info) # Normalize
            board.pop()
            
            if info and len(info) > 0 and 'score' in info[0]:
                ...
                quality = calculate_move_quality(centipawn_loss)
                
                player_qualities[player_name]['total_quality'] += quality
                player_qualities[player_name]['moves'] += 1
                detail['centipawn_loss'] = centipawn_loss
            else:
                logging.warning(f"analyze_player_quality received invalid info: {repr(raw_info)}")
        
        move_details_for_log.append(detail)
        board.push(move)
        
    return player_qualities, move_details_for_log

def estimate_rating_from_quality(model, quality_score, model_type):
    """Estimates Elo rating from a move quality score using the chosen model."""
    elo_search_range = np.arange(1000, 3500, 10).reshape(-1, 1)
    if model_type == 'Robust (Tukey)':
        X_search = sm.add_constant(elo_search_range)
        predicted_qualities = model.predict(X_search)
    else:
        predicted_qualities = model.predict(elo_search_range)
    closest_index = np.argmin(np.abs(predicted_qualities - quality_score))
    return elo_search_range[closest_index][0]

def log_move_by_move_analysis(game_num, game, move_details):
    """
    Creates a detailed text log from pre-analyzed move data.
    This function no longer uses the chess engine directly.
    """
    ...
    try:
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            for detail in move_details:
                f.write(f"Move: {detail['move_num_str']} {detail['move_san']} ({detail['player']})\n")
                if detail['centipawn_loss'] is not None:
                    f.write(f"  - Best Move: {detail['best_move_san']} (Eval: {detail['best_move_eval']/100.0:.2f})\n")
                    f.write(f"  - Centipawn Loss: {detail['centipawn_loss']}\n\n")
                else:
                    f.write("  - (No analysis for this position)\n\n")

        logging.info(f"Successfully created move-by-move log for Game {game_num}.")
    except Exception as e:
        # Using repr(e) gives a more detailed error message if something goes wrong.
...
def process_game(game_tuple):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(ENGINE_PATH) as engine:
            ...
            # Step 1: Generate move analyses for all positions.
            move_analyses = {}
            board = game.board()
            for move in game.mainline_moves():
            ...
            # Step 2: Analyze human player quality AND get details for logging in one go.
            player_results, move_details = analyze_player_quality_and_get_details(game, move_analyses, engine)

            # Step 3: Create the detailed log file from the results of Step 2.
            log_move_by_move_analysis(game_num, game, move_details)

            # Step 4: Filter analyses for the target player to build the model.
            ...
            # Step 5: Build performance models.
            ...
            if model_results[0] is None:
                ...
                return None
            model, r_squared, mse, ratings, quality_scores, model_type = model_results
            
            # Step 6: Plotting.
            plt.figure(figsize=(10, 6))
            plt.scatter(ratings, quality_scores, alpha=0.6, label="Engine Performance (Move Quality)")
            ...
            # Step 7: Package results (player_results was already computed in Step 2).
            game_data = {
        ...
        logging.error(f"A critical error occurred in worker for game {game_num}: {repr(e)}")
        return None
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            ...
            if PLAYER_TO_ANALYZE in game_data['player_results']:
                ...
                avg_quality = (results['total_quality'] / results['moves']) if results['moves'] > 0 else 0
                est_rating = estimate_rating_from_quality(game_data['model'], avg_quality, game_data['model_type'])
                f.write(f"  Player: {PLAYER_TO_ANALYZE}\n")
                f.write(f"    - Average Move Quality: {avg_quality:.2%}\n")
    ...
    try:
        ...
        for i, data in enumerate(all_games_data):
            ...
            if PLAYER_TO_ANALYZE in data['player_results']:
                ...
                pdf.set_font("Helvetica", '', 10)
                avg_quality = (results['total_quality'] / results['moves']) if results['moves'] > 0 else 0
                est_rating = estimate_rating_from_quality(data['model'], avg_quality, data['model_type'])
                
                pdf.multi_cell(w=0, h=5, text=f"  - Average Move Quality: {avg_quality:.2%}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            ...
            for game_data in results_iterator:
                ...
                if PLAYER_TO_ANALYZE in game_data['player_results']:
                    ...
                    if player_data['moves'] > 0:
                        avg_quality = player_data['total_quality'] / player_data['moves']
                        est_rating = estimate_rating_from_quality(game_data['model'], avg_quality, game_data['model_type'])
