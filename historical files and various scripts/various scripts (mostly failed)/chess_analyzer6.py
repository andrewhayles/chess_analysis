# chess_analyzer_uci.py
# A script to analyze PGN chess games and estimate player ratings.
# This version uses a single engine at various UCI-controlled strength
# levels to generate a precise performance model for each game.
...
ANALYSIS_TIME_LIMIT = 0.5  # Time in seconds for the oracle engine to find the best move.
...
def get_move_template(board, engine):
    ...
    try:
        # Use the engine at full strength to find the best moves
...
def build_model_with_uci_levels(engine, move_templates):
    """
    Generates performance data by running a single engine at various Elo levels.
    """
    logging.info("Building performance model using UCI strength levels...")
    
    # Define the Elo levels to test. Max is 3190, min is 1320 for this Stockfish version.
    elo_levels = np.arange(1400, 3200, 200)
    ratings_data = []
    hit_rates_data = []
    ...
    for elo in elo_levels:
        ...
        try:
            # Configure the engine to play at a specific strength
            engine.configure({"UCI_LimitStrength": True, "UCI_Elo": elo})
            
            hits, total_moves = 0, 0
            # Use a subset of positions for speed, e.g., first 50
    ...
    # Reset engine to full strength after testing
    engine.configure({"UCI_LimitStrength": False})

    if len(ratings_data) < 2:
        return None, None, None, None, None # Not enough data to build a model
    ...
    # Corrected this line to predict from ratings, not hit_rates
        ...
        with chess.engine.SimpleEngine.popen_uci(ENGINE_PATH) as engine:
            with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
                ...
                while True:
                    ...
                    # Step 2: Build the performance model using UCI strength levels
                    model_results = build_model_with_uci_levels(engine, move_templates)
