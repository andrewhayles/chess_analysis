...
# This version is OPTIMIZED to run faster by only building the performance model
# based on the target player's positions.
...
ANALYSIS_TIME_LIMIT = 5.0  # Increased for better accuracy
    ...
    for elo in elo_levels:
        ...
        try:
            ...
            for i, (fen, template) in enumerate(move_templates_for_model.items()):
                if i >= 50: break # Still cap at 50 positions for speed
        ...
        with chess.engine.SimpleEngine.popen_uci(ENGINE_PATH) as engine:
            with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
                ...
                while True:
                    ...
                    if PLAYER_TO_ANALYZE not in (game.headers.get("White"), game.headers.get("Black")):
                        continue

                    game_num += 1
                    logging.info(f"--- Processing Game {game_num}: {game.headers.get('White', '?')} vs. {game.headers.get('Black', '?')} ---")

                    # Step 1: Generate move templates for ALL positions
                    move_templates = {}
                    board = game.board()
                    for move in game.mainline_moves():
                    ...
                    # --- OPTIMIZATION: Filter templates for the target player's moves ---
                    player_color = chess.WHITE if game.headers.get("White") == PLAYER_TO_ANALYZE else chess.BLACK
                    player_specific_templates = {}
                    board = game.board()
                    for move in game.mainline_moves():
                        if board.turn == player_color:
                            player_specific_templates[board.fen()] = move_templates[board.fen()]
                        board.push(move)
                    # --- END OPTIMIZATION ---

                    # Step 2: Build the performance model using ONLY the player-specific positions
                    ...
                    if PLAYER_TO_ANALYZE in player_results:
                        player_data = player_results[PLAYER_TO_ANALYZE]
