...
#   3. INCREASED ROBUSTNESS: Better error handling and logging.
#   4. BUG FIX: Corrected the `analyze_player_quality` function.
#   5. PROGRESS FILE: Generates a clean `progress_summary.txt`.
#   6. LIVE PROGRESS BAR: Shows a real-time progress bar during long steps.
#   7. FINAL FIX: Player analysis now uses a fresh engine for each move to prevent state corruption.
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(engine_info['path']) as engine:
            ...
            for fen in fens:
                ...
                # Request only the score to avoid parsing errors with the PV
...
def analyze_player_quality(game, ground_truth_evals, analysis_engine_info):
    ...
    to prevent engine state corruption.
    """
    total_quality = 0
    moves_counted = 0
    ...
    for move in game.mainline_moves():
        # Only analyze moves made by the target player
        if board.turn == player_color:
            fen = board.fen()
            ...
            if best_eval is not None:
                try:
                    # Create a fresh engine process for this specific move analysis
                    with chess.engine.SimpleEngine.popen_uci(analysis_engine_info['path']) as engine:
                        ...
                        info = engine.analyse(temp_board, chess.engine.Limit(depth=PLAYER_ANALYSIS_DEPTH), info=chess.engine.INFO_SCORE)
                        if isinstance(info, list): info = info[0]

                        if 'score' in info:
                            # The POV is now from the player who just moved.
                            played_move_score = info['score'].pov(temp_board.turn).score(mate_score=30000)
                            # The ground truth POV was for the player about to move.
        ...
        # Push the move to the main board to advance the game state
        board.push(move)
