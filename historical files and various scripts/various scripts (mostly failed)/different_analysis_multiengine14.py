# chess_analyzer_multi_engine_robust.py
# A script to analyze PGN chess games and estimate a specific player's rating.
# This version uses a sophisticated multi-engine, depth-based approach and
# includes robust error handling for unstable or failing engines.
#
# KEY IMPROVEMENTS (from previous version):
#   1. PARTIAL ANALYSIS: If an engine fails or times out mid-game, the script
#      now uses the data from the moves it successfully completed, rather
#      than discarding all data from that engine for the game.
#   2. UNICODE ERROR HANDLING: Specifically catches and handles `UnicodeDecodeError`
#      caused by engines with non-standard startup messages, preventing crashes.
#   3. EFFICIENT WORKER: The model-building worker performs the entire
#      move-evaluate-compare loop in a single process.
#   4. RESOURCE MANAGEMENT: Parallel processing is capped to a sensible limit.
#   5. GRANULAR SAVING: Saves progress after each major step, allowing the
#      script to resume efficiently after an interruption.
#   6. LIVE PROGRESS BAR: Shows real-time progress for all long steps.
#   7. SEAMLESS RESUME: Uses a single, persistent output folder.
...
MODEL_BUILD_DEPTH = 11
PLAYER_ANALYSIS_DEPTH = 11 # Depth for analyzing the human player's moves

# TIMEOUTS (in seconds) to prevent hangs from misbehaving engines
ORACLE_TIMEOUT = 300
MODEL_BUILD_TIMEOUT = 300
PLAYER_ANALYSIS_TIMEOUT = 180
...
NUM_ORACLE_ENGINES = 4

# --- SCRIPT SETUP ---
SESSION_TIMESTAMP = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')

# --- Main Logger (for detailed debug info) ---
# Logs will now append to the same file in the persistent session folder
...
# Clear previous handlers to avoid duplicate logging on re-runs in the same process
...
def get_engine_quality_worker(args, ground_truth_evals):
    ...
    # Return the list of qualities, even if it's partial or empty
    ...
    if remaining_engines:
        ...
        with multiprocessing.Pool(processes=min(num_processes, multiprocessing.cpu_count())) as pool:
            for i, result in enumerate(pool.imap_unordered(worker_func, tasks)):
                ...
                if qualities_list:  # Check if the list of qualities is not empty
...
def analyze_player_quality(game, ground_truth_evals, analysis_engine_info):
    ...
    This version is robust, creating a new engine process for each move.
    """
    total_quality = 0
    moves_counted = 0
...
if __name__ == "__main__":
    # 'spawn' is a safer start method for multiprocessing on Windows and macOS
