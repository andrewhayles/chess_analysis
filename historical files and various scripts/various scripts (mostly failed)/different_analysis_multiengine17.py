# chess_analyzer_multi_engine_v19.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY IMPROVEMENTS (from v18):
#   1. EXTENSIBLE DATA FORMAT: The session file now permanently saves the raw
#      performance data for every model engine on a per-game basis.
#   2. FUTURE-PROOFING: This new format allows a future "enhancer" script to add
#      new engine data points without needing to re-run the entire analysis,
#      enabling incremental model improvement.
...
MODEL_BUILD_DEPTH = 9
PLAYER_ANALYSIS_DEPTH = 9 # This MUST match the model build depth

# TIMEOUTS (in seconds) to prevent hangs from misbehaving engines
ORACLE_TIMEOUT = 300
MODEL_BUILD_TIMEOUT = 6
PLAYER_ANALYSIS_TIMEOUT = 6
...
def build_model_with_real_engines(ground_truth_evals, model_engines, num_processes, session_data, pgn_path):
    ...
    # Return the raw qualities dictionary for permanent storage
...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data['games_to_process_indices']:
            ...
            # *** KEY CHANGE: Store the raw model data for future enhancement ***
            game_data_for_session = {
                ...
                'model_results_raw': raw_model_results # This saves the crucial data
            }
            session_data['completed_games_data'].append(game_data_for_session)
            session_data['games_to_process_indices'].pop(0)
            session_data['in_progress_game'] = {} # Clear the in-progress game
            save_session(session_data, pgn_path)
