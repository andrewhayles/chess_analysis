...
#   7. FINAL FIX: Player analysis now only requests the score to prevent PV parsing errors.
...
ORACLE_ANALYSIS_DEPTH = 5
MODEL_BUILD_DEPTH = 5
PLAYER_ANALYSIS_DEPTH = 5 # Depth for analyzing the human player's moves
            ...
            for move in game.mainline_moves():
                if board.turn == player_color:
                    ...
                    if best_eval is not None:
                        board.push(move)
                        # *** FIX: Request only the score to prevent PV parsing errors ***
                        info = engine.analyse(board, chess.engine.Limit(depth=PLAYER_ANALYSIS_DEPTH), info=chess.engine.INFO_SCORE)
                        if isinstance(info, list): info = info[0]
