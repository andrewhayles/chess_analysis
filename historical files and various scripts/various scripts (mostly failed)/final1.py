...
def get_standard_engine_options(csv_options_str, engine_info):
    ...
    if csv_options_str:
        try:
            ...
            logging.warning(f"Could not decode JSON options: {csv_options_str}")
    return options
...
def universal_worker(args):
    ...
    try:
        ...
        logging.error(f"Worker '{worker_mode}' for {engine_info['name']} failed on FEN {fen}: {e}")
    return None
