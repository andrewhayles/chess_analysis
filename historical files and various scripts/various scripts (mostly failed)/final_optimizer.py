...
#   - Live Progress Logging: Appends detailed updates, including R² leaderboards,
...
#   - Configurable Start Move: Analysis can be configured to start from a specific move number.
#
# --- FRAMEWORKS ---
...
PROGRESS_LOG_FILE = SESSION_FOLDER / "live_progress_log.txt"
...
POSITIONS_PER_GAME_TO_SAMPLE = 5
ANALYSIS_START_MOVE = 10  # <--- NEW: Start analysis from this move number.
...
def log_progress_update(leaderboard_text, game_info, session_data):
    """Appends a detailed progress update to the live log file."""
    with open(PROGRESS_LOG_FILE, 'a') as f:
        f.write(f"--- PROGRESS UPDATE: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---\n")
        f.write(f"Last Game Processed: {game_info['id']} ({game_info['white']} vs. {game_info['black']})\n")
        f.write(f"Total Games Analyzed: {len(session_data.get('processed_game_ids', []))}\n")
        f.write(f"Total Positions in Dataset: {len(session_data.get('all_ground_truth', {}))}\n")
        f.write("\n--- LEADERBOARD ---\n")
...
def get_ground_truth_for_game(game, oracle_engine, pool):
    """
    Identifies candidate positions from a game for the specified player,
    starting from the move number defined in ANALYSIS_START_MOVE.
    """
    player_color = chess.WHITE if game.headers.get("White", "?").lower() == PLAYER_TO_ANALYZE.lower() else chess.BLACK
    
    # --- MODIFIED: Use ANALYSIS_START_MOVE to determine the starting point ---
    # The ply index is calculated from the move number. Move 10 corresponds to ply indices 18 and 19.
    start_ply_index = (ANALYSIS_START_MOVE - 1) * 2
    candidate_nodes = [
        node for i, node in enumerate(game.mainline()) 
        if node.board().turn == player_color and i >= start_ply_index
    ]
    # --- END MODIFICATION ---
...
def run_data_collection(pgn_path, oracle_engine, model_engines, session_data):
...
def run_analysis_mode(all_engines, pgn_path, oracle_engine, model_engines, session_data):
    constants = load_json_file(CONSTANTS_FILE, DEFAULT_CONSTANTS)
    logging.info(f"Starting analysis with constants: {constants}")
    for updated_session_data, game_info in run_data_collection(pgn_path, oracle_engine, model_engines, session_data):
    ...
    logging.info("--- Analysis of all games complete. ---")
...
def run_optimization_mode(all_engines, pgn_path, oracle_engine, model_engines, session_data):
    logging.info("--- Starting Optimization Mode ---")
    logging.info("Step 1: Collecting simulation data from the training PGN...")
    for _, _ in run_data_collection(pgn_path, oracle_engine, model_engines, session_data):
        pass
...
def main():
    ...
    print("  1. Analyze PGN (Resumable, Live Leaderboard)")
    print("  2. Optimize Constants (Resumable, finds best constants)")
    ...
    if session_data.get('pgn_file') != str(pgn_path) or session_data.get('script_version') != 'v8':
        logging.warning("New PGN/script version detected or no session found. Starting a new session.")
        session_data = {'pgn_file': str(pgn_path), 'script_version': 'v8'}

    if mode == '1':
        run_analysis_mode(all_engines, pgn_path, oracle_engine, model_engines, session_data)
    elif mode == '2':
        run_optimization_mode(all_engines, pgn_path, oracle_engine, model_engines, session_data)
    else:
