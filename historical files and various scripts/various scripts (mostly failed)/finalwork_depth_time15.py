# chess_analyzer_multi_engine_v37_silent.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGES (from v36):
#   1. SUPPRESS ENGINE STDERR: The script now redirects the standard error
#      output of the engine processes to DEVNULL, preventing engine-specific
#      startup messages (like from Lc0) from cluttering the analysis log.
...
def universal_worker(args):
    ...
    try:
        # MODIFIED: Added stderr=subprocess.DEVNULL to suppress engine's own output
...
def run_engine_simulations(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, is_checkpoint=False):
    """
    Runs engine simulations sequentially for each engine, using multiple cores
    to process the positions for that single engine.
    """
    ...
    # --- REFACTORED LOGIC: Process one engine at a time ---
    total_engines = len(engines_to_process)
    for engine_idx, engine_info in enumerate(engines_to_process):
        engine_name = engine_info['name']
        
        # Check if this engine's data already exists and is complete
        is_complete = True
        if not all(engine_name in simulation_results.get(fen, {}) for fen in ground_truth_template.keys()):
            is_complete = False
        ...
        logging.info(f"--- ({engine_idx + 1}/{total_engines}) Analyzing with {engine_name} ---")
        ...
        # Use the pool for the tasks of this single engine
