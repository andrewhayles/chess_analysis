# chess_analyzer_multi_engine_v39_live_logging.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGES (from v38):
#   1. LIVE ANALYSIS LOGGING: A new 'live_analysis_log.txt' is created to
#      provide real-time updates during the analysis of a single game.
#   2. INCREMENTAL REPORTING: After each engine completes its analysis, the
#      script immediately calculates the scores and R-squared value for the
#      data collected so far and appends a detailed report to the live log.
#   3. ENHANCED MONITORING: This allows for close monitoring of long-running
#      analyses, showing the model's R² as it evolves with each added engine.
...
def log_live_analysis_update(game_info, engine_info, current_scores, current_sim_results, all_engines, template_size):
    """Appends a live update to a detailed log file after each engine completes analysis."""
    log_file_path = SESSION_FOLDER / "live_analysis_log.txt"
    
    # Get all engines that have results so far
    processed_engine_names = set(current_scores.keys())
    engines_for_model = [e for e in all_engines if e['name'] in processed_engine_names]

    # Build a temporary model with the current data
    model_bundle = build_model_from_scores(current_scores, engines_for_model)
    current_r_squared = model_bundle['r_squared'] if model_bundle else -1.0
    
    with open(log_file_path, 'a', encoding='utf-8') as f:
        f.write(f"--- LIVE UPDATE: Game {game_info['num']} ({game_info['white']} vs {game_info['black']}) ---\n")
        f.write(f"Timestamp: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Last Engine Completed: {engine_info['name']} (Rating: {engine_info['rating']})\n")
        f.write(f"Score for this Engine: {current_scores.get(engine_info['name'], 'N/A'):.4f}\n")
        f.write(f"Current Model R² (with {len(engines_for_model)} engines): {current_r_squared:.4f}\n")
        
        f.write(f"Moves by {engine_info['name']}:\n")
        for fen, moves in current_sim_results.items():
            played_move = moves.get(engine_info['name'])
            if played_move:
                f.write(f"  - FEN: {fen}, Move: {played_move}\n")
        f.write("-" * 50 + "\n\n")
...
def run_engine_simulations(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, all_engines, game_info, chosen_template_size, is_checkpoint=False):
    """
    Runs engine simulations sequentially, logging results after each engine.
    """
    ...
    for engine_idx, engine_info in enumerate(engines_to_process):
        ...
        if is_complete:
            logging.info(f"({engine_idx + 1}/{total_engines}) Data for {engine_name} already exists. Skipping.")
            continue
        ...
        if not is_checkpoint:
            current_scores = calculate_scores_from_sims(simulation_results, ground_truth_template, chosen_template_size)
            log_live_analysis_update(game_info, engine_info, current_scores, simulation_results, all_engines, chosen_template_size)
        ...
        while session_data.get('games_to_process_indices'):
            ...
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                ...
                for strategy_name, params in analysis_strategies.items():
                    run_engine_simulations(ground_truth_template, checkpoint_engines, session_data, pgn_path, params, all_engines, game_info, 1, is_checkpoint=True)
            ...
            if final_engines_to_run:
                 ...
                 run_engine_simulations(ground_truth_template, final_engines_to_run, session_data, pgn_path, chosen_strategy_params, all_engines, game_info, chosen_template_size)
            else:
