# chess_analyzer_multi_engine_v40_live_logging_all.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGES (from v39):
#   1. UNCONDITIONAL LIVE LOGGING: The script now logs analysis updates after
#      every single engine run, including those in the initial "checkpoint" phase.
#   2. CLEARER LOGS: The 'live_analysis_log.txt' now specifies whether an
#      update is from the 'Checkpoint Phase' or the 'Main Analysis' phase.
...
def log_live_analysis_update(game_info, engine_info, current_scores, current_sim_results, all_engines, template_size, is_checkpoint_run=False):
...
def run_engine_simulations(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, all_engines, game_info, chosen_template_size, is_checkpoint=False):
    ...
    for engine_idx, engine_info in enumerate(engines_to_process):
        ...
        # --- LIVE LOGGING HOOK (NOW UNCONDITIONAL) ---
        # This will now log after every engine run, regardless of phase.
        current_scores = calculate_scores_from_sims(simulation_results, ground_truth_template, chosen_template_size)
        log_live_analysis_update(game_info, engine_info, current_scores, simulation_results, all_engines, chosen_template_size, is_checkpoint_run=is_checkpoint)
