...
def run_engine_simulations(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, is_checkpoint=False):
    ...
    if sim_results_key in session_data['in_progress_game']:
        existing_fens = session_data['in_progress_game'][sim_results_key].keys()
        if set(ground_truth_template.keys()).issubset(existing_fens):
...
def calculate_scores_from_results(simulation_results, ground_truth_template, template_size):
    ...
    for fen, moves_by_engine in simulation_results.items():
        ...
        for engine_name, move_uci in moves_by_engine.items():
            if move_uci in template_moves:
                ...
                # This part is complex as it requires re-analyzing to get the score of the missed move.
                # For simplicity in this refactoring, we'll assign a fixed low score for a miss.
                # A more accurate approach would require another engine call here.
                engine_scores[engine_name].append(0.0) # Simplified punishment for a miss
    
    avg_scores = {name: np.mean(scores) if scores else 0.0 for name, scores in engine_scores.items()}
    return avg_scores
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                ...
                # Step 1: Run depth-first simulation
                run_engine_simulations(ground_truth_template, checkpoint_engines, session_data, pgn_path, analysis_strategies['depth_first'], is_checkpoint=True)
                depth_sim_results = session_data['in_progress_game'][f"model_engine_moves_{analysis_strategies['depth_first']['name']}"]

                # Step 2: Test depth-first with multiple template sizes
                checkpoint_results = []
                for t_size in range(1, TEMPLATE_MOVE_COUNT + 1):
                    scores = calculate_scores_from_results(depth_sim_results, ground_truth_template, t_size)
                    model_bundle = build_model_from_scores(scores, checkpoint_engines)
                    if model_bundle:
                        checkpoint_results.append({'r2': model_bundle['r_squared'], 'strategy': 'depth_first', 'template_size': t_size})

                # Step 3: If best depth R² is low, test time-first
                best_depth_result = max(checkpoint_results, key=lambda x: x['r2']) if checkpoint_results else None
                if not best_depth_result or best_depth_result['r2'] < R_SQUARED_THRESHOLD:
                    logging.info(f"Best depth-first R² ({best_depth_result['r2'] if best_depth_result else 'N/A'}) is below threshold. Testing time-first strategy.")
                    run_engine_simulations(ground_truth_template, checkpoint_engines, session_data, pgn_path, analysis_strategies['time_first'], is_checkpoint=True)
                    time_sim_results = session_data['in_progress_game'][f"model_engine_moves_{analysis_strategies['time_first']['name']}"]
                    for t_size in range(1, TEMPLATE_MOVE_COUNT + 1):
                        scores = calculate_scores_from_results(time_sim_results, ground_truth_template, t_size)
                        model_bundle = build_model_from_scores(scores, checkpoint_engines)
                        if model_bundle:
                            checkpoint_results.append({'r2': model_bundle['r_squared'], 'strategy': 'time_first', 'template_size': t_size})
                
                # Step 4: Determine the best overall strategy and template size
            ...
            # Step 5: Run final analysis with the chosen parameters
            ...
            final_scores = calculate_scores_from_results(final_sim_results, ground_truth_template, chosen_template_size)
