# chess_analyzer_multi_engine_v21_hybrid.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGE (from v20):
#   1. HYBRID ANALYSIS CONTROL: This version implements a hybrid control system as requested.
#      - Oracle Analysis (Ground Truth): Uses a fixed depth with a time-based safety net.
#      - Model & Player Analysis: Uses a fixed time limit.
...
def get_averaged_ground_truth(game, oracle_engines, num_processes, session_data, pgn_path):
    ...
    # Pass depth and timeout to the oracle worker
...
def build_model_with_real_engines(ground_truth_evals, model_engines, num_processes, session_data, pgn_path):
    ...
    if remaining_engines:
        ...
        # Pass the fixed time to the model building worker
