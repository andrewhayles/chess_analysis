# chess_analyzer_multi_engine_v23_sequential_engines.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGE (from v22):
#   1. SEQUENTIAL ENGINE PROCESSING: The script now processes one engine strength
#      level at a time.
#   2. FOCUSED PARALLELISM: For each engine strength level, it uses all specified
#      cores to analyze the moves for that single engine in parallel. This ensures
#      processors are focused on one task before moving to the next.
