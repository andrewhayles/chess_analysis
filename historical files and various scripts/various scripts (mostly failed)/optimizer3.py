...
# >>> NEW: Set the number of model-building engines to use.
# They will be selected evenly from the full list, sorted by rating.
# Set to None to use all available engines.
NUM_MODEL_ENGINES_TO_USE = 30
...
NUM_ANALYSIS_CORES = multiprocessing.cpu_count() # Use all available CPU cores
...
def run_engine_simulations_for_timeout(ground_truth_template, model_engines, timeout):
    ...
    logging.info(f"----- TESTING TIMEOUT: {timeout} seconds ({len(model_engines)} engines) -----")
...
def select_engines_evenly(engine_list, num_to_select):
    """Selects a specified number of engines, evenly distributed by their list order."""
    if num_to_select is None or num_to_select >= len(engine_list):
        return engine_list # Return all if selection is not smaller
    
    # Calculate indices for even distribution
    indices = np.linspace(0, len(engine_list) - 1, num_to_select, dtype=int)
    
    # Ensure unique indices, especially for small lists
    unique_indices = sorted(list(set(indices)))
    
    selected = [engine_list[i] for i in unique_indices]
    return selected
...
def main():
    ...
    # --- FIX: Sort all engines by rating in descending order (highest first) ---
    all_engines.sort(key=lambda x: x['rating'], reverse=True)

    # The first engine in the list is now the highest rated, used for the oracle
    oracle_engines = all_engines[:NUM_ORACLE_ENGINES]
    # The rest of the engines are available for model building
    available_model_engines = all_engines[NUM_ORACLE_ENGINES:]

    # Select a subset of model engines if specified
    model_engines = select_engines_evenly(available_model_engines, NUM_MODEL_ENGINES_TO_USE)
    
    logging.info(f"Loaded {len(oracle_engines)} oracle engine(s): {[e['name'] for e in oracle_engines]}")
    logging.info(f"Selected {len(model_engines)} model-building engines (from {len(available_model_engines)} available).")
    if len(model_engines) < 20: # Log the names if the list is short
        logging.info(f"Engines being used for modeling: {[e['name'] for e in model_engines]}")
        ...
        while len(games_to_optimize) < NUM_GAMES_TO_OPTIMIZE:
            try:
                ...
                if game is None: 
                    ...
                    break
                # Check if the player is in the game headers
                white_player = game.headers.get("White", "?")
                black_player = game.headers.get("Black", "?")
                if PLAYER_TO_ANALYZE.lower() in (white_player.lower(), black_player.lower()):
                    games_to_optimize.append(game)
                    logging.info(f"Added game {len(games_to_optimize)}/{NUM_GAMES_TO_OPTIMIZE}: {white_player} vs {black_player}")
            except Exception as e:
