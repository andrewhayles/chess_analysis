...
def calculate_rating_from_scores(scores_df, player_score, benchmark_engine_info):
    ...
    # Ensure we have scores for benchmark engines
...
def main():
    ...
    if not items_to_analyze:
        ...
        for item in items_to_analyze:
            ...
            # Append to file for safety
            log_entry_df = pd.DataFrame([{'fen': fen, 'engine_name': engine_name_for_log, 'score': score}])
            log_entry_df.to_csv(GRANULAR_LOG_PATH, mode='a', header=not granular_header_written, index=False)
            granular_header_written = True
            
            # Append to in-memory dataframe for live calculation
            full_log_df = pd.concat([full_log_df, log_entry_df], ignore_index=True)
            ...
            if len(items_for_this_fen) == num_required_per_pos:
                # This position is now fully analyzed, so calculate and log status
                ...
                if not inst_player_score_row.empty:
                    ...
                    # Calculate overall stats from the full in-memory log
