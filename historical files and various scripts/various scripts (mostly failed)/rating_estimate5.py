...
def main():
    ...
    # --- 3. Check for existing progress and resume ---
        ...
        try:
            ...
            if not log_df.empty:
                ...
                for name in engine_names:
                    # Check if column exists before summing
                    if f'{name}_score' in log_df.columns:
                        running_totals[name]['score_sum'] = log_df[f'{name}_score'].sum()
                        running_totals[name]['count'] = len(log_df)
                if 'player_score' in log_df.columns:
    ...
    if not positions_to_analyze and len(completed_fens) > 0:
        print("All positions have already been analyzed. Generating final report.")
    elif not positions_to_analyze:
        print("No new positions to analyze.")
    else:
    ...
    if positions_to_analyze:
        print("Initializing engines for analysis...")
        ...
        for i, pos_data in enumerate(positions_to_analyze):
            ...
            # --- *** MODIFIED LOGIC *** ---
            # First, calculate the rating if possible
            avg_scores_data = []
            ...
            if len(avg_scores_data) >= 2:
                ...
                # *** FIX ***: Only attempt regression if there is more than one unique score value
            ...
            # Second, add the running average to the results for this position
            current_pos_results['player_running_avg_rating'] = f"{estimated_rating:.0f}" if estimated_rating is not None else "N/A"
            
            # Third, log the detailed results for this position to the CSV (ensures progress is saved)
            ...
            # Finally, write the live status log
    ...
    if len(engine_plot_df) >= 2 and not player_plot_df.empty:
        # Only do final regression if there's variance
        if engine_plot_df['avg_score'].nunique() > 1:
