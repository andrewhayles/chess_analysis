...
def main():
    ...
    if os.path.exists(OUTPUT_LOG_PATH):
        try:
            ...
            if not log_df.empty:
                ...
                print(f"Found {len(completed_fens)} previously completed positions in '{OUTPUT_LOG_PATH}'.")
        except (pd.errors.EmptyDataError, KeyError):
             print(f"'{OUTPUT_LOG_PATH}' is empty or malformed. Starting fresh.")
             completed_fens = set()
    
    # --- 3. *** NEW *** Phase 1: Update Oracle Cache ---
    ...
    if not positions_to_analyze:
        ...
        print(f"\n--- Phase 2: Analyzing {len(positions_to_analyze)} new positions with benchmark engines ---")
        benchmark_engines = {}
        ...
        pbar = tqdm(total=total_analyses, desc="Benchmark Analysis", unit="analysis")
        ...
        for i, pos_data in enumerate(positions_to_analyze):
            ...
            # Add current results to running totals before calculating new average
            running_totals['player']['score_sum'] += player_score
            running_totals['player']['count'] += 1
            for name in benchmark_engines.keys():
                running_totals[name]['score_sum'] += current_pos_results[f'{name}_score']
