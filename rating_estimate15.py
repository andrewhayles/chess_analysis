...
import os
import json
import random  # --- MODIFICATION: Added for random sampling ---
...
# --- MODIFICATION: Updated this function for random selection ---
def get_positions_from_pgn(pgn_path, player_name, positions_per_game, start_move):
    ...
    It finds all valid moves for the player in a game and then randomly samples
    the specified number of positions from them.
    """
    ...
    with open(pgn_path, 'r', errors='ignore') as pgn:
        while True:
            try:
                ...
                board = game.board()
                potential_positions_this_game = []
                is_player_white = player_name in white_player

                # Step 1: Collect all possible positions from the game
                for move in game.mainline_moves():
                    ...
                    if is_player_turn and board.fullmove_number >= start_move:
                        potential_positions_this_game.append({"fen": board.fen(), "actual_move": move.uci()})
                    
                    board.push(move)

                # Step 2: Randomly sample from the collected positions
                if positions_per_game is not None and len(potential_positions_this_game) > 0:
                    num_to_sample = min(len(potential_positions_this_game), positions_per_game)
                    selected_positions = random.sample(potential_positions_this_game, num_to_sample)
                    final_positions.extend(selected_positions)
                elif positions_per_game is None: # If no limit, take all collected positions
                    final_positions.extend(potential_positions_this_game)
