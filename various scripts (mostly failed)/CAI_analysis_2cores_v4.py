...
def optimize_and_compare():
    ...
    try:
        ...
        # Handle the optional 'executable_name' column
        if 'executable_name' not in df.columns:
            df['executable_name'] = df['engine_name'] # Fallback to engine_name
        else:
            # FIXED: Use modern pandas syntax to avoid FutureWarning
    ...
    # REMOVED: Confusing ORACLE_ENGINE_NAME setting.
    # The Oracle is now always the highest-rated engine from the CSV.
