...
# --- Analysis Settings ---
# Analysis will stop when EITHER the depth OR the timeout is reached.
ANALYSIS_DEPTH = 18 # The target depth for analysis.
# NEW: Timeout in seconds for a single engine analysis.
...
def calculate_cai_for_move(oracle_engine, board, test_engine_move, oracle_best_move_analysis, constants):
    ...
    # UPDATED: Use timeout for this analysis as well.
    limit = chess.engine.Limit(depth=ANALYSIS_DEPTH, time=ANALYSIS_TIMEOUT_SECONDS)
    info_after_test_move = oracle_engine.analyse(board, limit)
...
def analyze_position_worker(job):
    ...
    try:
        ...
        # Suppress engine startup messages (like the LC0 banner)
        ...
        # UPDATED: Create a single Limit object to use for all analyses.
        limit = chess.engine.Limit(depth=ANALYSIS_DEPTH, time=ANALYSIS_TIMEOUT_SECONDS)
        
        test_engine_result = test_engine.play(board, limit)
        test_engine_move = test_engine_result.move

        oracle_analysis = oracle_engine.analyse(board, limit, multipv=2)
        ...
        # Robustly quit engines, ignoring errors if they already crashed.
        if oracle_engine:
            try:
                oracle_engine.quit()
...
def optimize_and_compare():
    ...
    try:
        ...
        if 'executable_name' not in df.columns:
            df['executable_name'] = df['engine_name']
        else:
            df['executable_name'] = df['executable_name'].fillna(df['engine_name'])
    ...
    for i, constants in enumerate(constant_combinations):
        ...
        if failed_analyses:
            ...
            for engine_name, error_msg, path_attempted in failed_analyses:
                error_summary[f"'{engine_name}' (Path: {path_attempted}): {error_msg}"] += 1
