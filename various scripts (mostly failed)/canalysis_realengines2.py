...
#      consensus on the single best move, avoiding MultiPV errors.
...
def engine_worker(task_queue, result_queue):
    ...
    while True:
        try:
            task_type, data, engine_info = task_queue.get()
            ...
            logging.error(f"Error in engine_worker task for {engine_info.get('name', 'N/A')}: {repr(e)}")
            # Signal failure
            if task_type == "get_best_move":
...
def get_weighted_template(game, oracle_engines):
    ...
    for _ in range(num_tasks):
        try:
            ...
            if best_move:
                ...
                position_voters[fen].append(engine_info['name'])
        except queue.Empty:
    ...
    final_templates = {}
    strongest_oracle_name = oracle_engines[0]['name']
    for fen, votes in position_votes.items():
        if votes:
            ...
            # If there's a clear winner, that's the template
            if len(top_voted_moves) == 1:
                final_templates[fen] = {top_voted_moves[0]}
            else:
                # On a tie, the vote from the strongest engine wins
                # We need to find which of the tied moves was voted by the strongest engine
                found_tie_breaker = False
                for i, oracle in enumerate(oracle_engines):
                    # Check if this engine's vote is one of the tied moves
                    # This requires getting the vote again, which is inefficient but necessary here
                    # A better design would store the vote alongside the voter
                    # For now, we assume the first engine is the tie-breaker
                    # This part is complex, so we simplify: pick the first of the tied moves
                    final_templates[fen] = {top_voted_moves[0]}
                    logging.warning(f"Tie detected for FEN {fen}. Defaulting to {top_voted_moves[0]}.")
                    break
