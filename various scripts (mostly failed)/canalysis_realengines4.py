...
#
# KEY FEATURES:
#   1. FAULT-TOLERANT ENGINE POOL: Assumes engines may fail. Uses per-task
#      timeouts and robust error handling to prevent crashes.
...
def engine_worker(task_queue, result_queue):
    ...
    while True:
        try:
            ...
            logging.error(f"Critical error in engine_worker task for {engine_info.get('name', 'N/A') if engine_info else 'N/A'}: {repr(e)}")
...
def get_weighted_template(game, oracle_engines, num_processes):
    ...
    for _ in range(num_tasks):
        try:
            _, fen, top_moves, _ = result_queue.get(timeout=ANALYSIS_TIME_LIMIT * 4)
