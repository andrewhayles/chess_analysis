...
#
# KEY FEATURES:
#   1. MAXIMUM STABILITY: Uses a "one task, one process" model. Each engine
#      analysis is launched in a completely isolated process to ensure that a
#      single engine crash cannot destabilize the entire program.
...
def get_weighted_template(game, oracle_engines, num_processes):
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        results = pool.map(oracle_analysis_worker, tasks)
        for fen, top_moves, engine_info in results:
            if top_moves:
...
def build_model_with_real_engines(player_specific_templates, model_engines, num_processes):
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        results = pool.map(model_engine_worker, tasks)
        for engine_info, hit_rate in results:
