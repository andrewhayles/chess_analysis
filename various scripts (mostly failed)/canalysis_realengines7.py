...
#      results queue (imap_unordered) and per-task timeouts to make the system
...
def get_weighted_template(game, oracle_engines, num_processes):
    ...
    try:
        ...
        for i in range(len(tasks)):
            try:
                # Apply a timeout to getting each result
                fen, top_moves, engine_info = result_iterator.next(timeout=ANALYSIS_TIME_LIMIT * 4)
                if top_moves:
                ...
                logging.error("Timeout waiting for an oracle worker to complete. Skipping task.")
            except Exception as e:
...
def build_model_with_real_engines(player_specific_templates, model_engines, num_processes):
    ...
    try:
        ...
        for i in range(len(tasks)):
            try:
                engine_info, hit_rate = result_iterator.next(timeout=MODEL_BUILD_TIME_LIMIT * 55)
                if hit_rate is not None:
                    ratings_data.append(engine_info['rating'])
                    hit_rates_data.append(hit_rate)
            except multiprocessing.TimeoutError:
                logging.error("Timeout waiting for a model worker to complete. Skipping task.")
            except Exception as e:
