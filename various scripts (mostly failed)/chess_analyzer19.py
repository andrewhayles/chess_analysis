...
# This version uses a robust, persistent engine pool architecture to ensure
# stability during long analysis runs.
#
# KEY FEATURES:
#   1. STABLE ENGINE POOL: Creates a single, persistent pool of engine processes
#      per game to handle all analysis tasks, preventing deadlocks.
#   2. DYNAMIC HIT COUNT: Defines a "hit" using a multi-move template and a
#      dynamic centipawn tolerance that adapts to the position's evaluation.
#   3. DEEP ANALYSIS: Uses a 6-second time limit for both the oracle and the
#      model-building engines to ensure high-quality data.
...
def process_one_game(game, game_num):
    ...
    try:
        ...
        for _ in range(len(fens_to_analyze)):
            try:
                _, fen, template = result_queue.get(timeout=ANALYSIS_TIME_LIMIT * 3)
        ...
        for _ in range(len(elo_levels)):
            try:
                _, elo, hit_rate = result_queue.get(timeout=MODEL_BUILD_TIME_LIMIT * 55)
