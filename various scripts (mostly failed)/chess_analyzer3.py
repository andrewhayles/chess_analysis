...
# This version includes a workaround for the 'mate_ok' bug.
...
def get_move_template(board, engine):
    ...
    try:
        ...
        # --- REPLACEMENT CODE BLOCK to avoid 'mate_ok' ---
        top_score_obj = info[0]['score'].white()

        if top_score_obj.is_mate():
            # If the top move is a mate, only other mating moves are acceptable.
        ...
        # If we get here, the top move is a centipawn score.
        # The old API call for this is just .score()
        ...
        for i in range(1, len(info)):
            ...
            if current_score_obj:
                # We only compare other centipawn scores
                if not current_score_obj.white().is_mate():
...
def analyze_player_hits(game, move_templates):
    ...
    for move in game.mainline_moves():
        ...
        if template_for_pos:
            ...
            if player_name not in player_hits: # Handle players with inconsistent naming
            ...
            with open(pgn_path) as pgn_file:
                ...
                while True:
                    try:
                        ...
                        logging.warning(f"Could not parse a game from PGN. Error: {e}. Skipping.")
                        continue
