# chess_analyzer_multi_engine_v16.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY IMPROVEMENTS (from v15):
#   1. DUAL MODELING: The script now trains both a Linear Regression and an
#      Isotonic Regression model for each game.
#   2. AUTOMATIC MODEL SELECTION: It calculates the R-squared value for both models
#      and automatically uses the one with the better fit for rating estimation.
#   3. ENHANCED REPORTING: Logs and graphs now specify which model type was
#      chosen (Linear or Isotonic) for each game's analysis.
...
PLAYER_ANALYSIS_DEPTH = 12 # This MUST match the model build depth
...
def build_model_with_real_engines(ground_truth_evals, model_engines, num_processes, session_data, pgn_path):
    """
    Builds the performance model by training both Linear and Isotonic models
    and selecting the one with the higher R-squared value.
    """
    ...
    # --- Dual Model Training and Selection ---
    ...
    # 2. Isotonic Model
    # Isotonic regression requires X to be sorted.
    sort_indices = np.argsort(ratings.flatten())
    ratings_sorted = ratings.flatten()[sort_indices]
    quality_scores_sorted = quality_scores[sort_indices]
    
    isotonic_model = IsotonicRegression(y_min=0.0, y_max=1.0, out_of_bounds="clip")
    isotonic_model.fit(ratings_sorted, quality_scores_sorted)
    isotonic_predictions = isotonic_model.predict(ratings.flatten())
    isotonic_r2 = r2_score(quality_scores, isotonic_predictions)

    # 3. Compare and select the best model
    if isotonic_r2 > linear_r2:
        logging.info(f"Model selected: Isotonic Regression. R-squared: {isotonic_r2:.4f} (vs. Linear R²: {linear_r2:.4f})")
        progress_logger.info(f"  -> Model created (Isotonic) with R-squared: {isotonic_r2:.4f}")
        return isotonic_model, isotonic_r2, ratings, quality_scores, "Isotonic"
    else:
        logging.info(f"Model selected: Linear Regression. R-squared: {linear_r2:.4f} (vs. Isotonic R²: {isotonic_r2:.4f})")
        progress_logger.info(f"  -> Model created (Linear) with R-squared: {linear_r2:.4f}")
        return linear_model, linear_r2, ratings, quality_scores, "Linear"
...
def estimate_rating_from_quality(model, quality_score, model_type):
    ...
    if model_type == "Linear":
        ...
        # For Isotonic, we can't solve directly. We search for the rating.
        search_ratings = np.arange(800, 3500, 1) # A wide range of possible ratings
        predicted_qualities = model.predict(search_ratings)
        # Find the index of the rating that gives the closest quality score
...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data['games_to_process_indices']:
            ...
            model, r_squared, ratings, quality_scores, model_type = model_results

            plt.figure(figsize=(10, 6))
            plt.scatter(ratings.flatten(), quality_scores, alpha=0.7, label="Engine Performance (Move Quality)")
            
            # Generate points for the model line
            plot_ratings_x = np.linspace(ratings.min(), ratings.max(), 200)
