...
#
# The script has been significantly modified from the original to support framework comparison.
...
def convert_eval_to_win_prob(evaluation_cp):
    ...
    if evaluation_cp is None:
        return 0.5
    # The formula from the document: WP(E) = 0.5 + 0.5 * (2 / (1 + e^(-k1*E)) - 1)
    # This simplifies to the standard logistic function scaled to [0, 1]
...
def calculate_rwpl_score(wp_loss):
    ...
    # Formula: RWPL Score = 100 * (1 - 2 * (WPLoss)^k2)
    # We must cap WPLoss at 0.5 to keep the score in [-100, 100]
    # A WPLoss > 0.5 means going from a winning to a losing position, which should be -100.
...
def calculate_cai_score(wp_loss, criticality):
    ...
    # Formula: Impact = WPLoss * (1 + w * Criticality)
    impact = wp_loss * (1 + W_CAI * criticality)
    # The final mapping to [-100, 100] is ambiguous in the text.
    # We will use a similar mapping to RWPL for consistency.
    capped_impact = min(impact, 0.5)
    return 100 * (1 - 2 * (capped_impact ** K2_RWPL)) # Using K2_RWPL for mapping
...
def calculate_move_scores(best_move_eval, played_move_eval, second_best_move_eval, top_n_evals):
    ...
    Returns:
        dict: A dictionary of scores for the move, e.g., {'RWPL': 85.4, 'CAI': 72.1, ...}
    """
    if played_move_eval is None or best_move_eval is None:
        return {}

    # 1. Calculate Win Probability Loss (WPLoss) - The basis for RWPL and CAI
    ...
    # 2. Calculate RWPL Score
    scores['RWPL'] = calculate_rwpl_score(wp_loss)

    # 3. Calculate CAI Score
    criticality = 0.0
    if second_best_move_eval is not None:
        eval_gap = abs(best_move_eval - second_best_move_eval)
        # Formula: Criticality = min(1, 2 / (1 + exp(-k3 * Δ)) - 1)
        # This is another way of writing a scaled sigmoid.
        criticality = 1 / (1 + np.exp(-K3_CAI * eval_gap))

    scores['CAI'] = calculate_cai_score(wp_loss, criticality)
    scores['Impact'] = scores['CAI'] # Impact_Score from MFQ is the CAI score

    # 4. Calculate Complexity Score (from MFQ)
    if top_n_evals and len(top_n_evals) > 1:
        scores['Complexity'] = np.std(top_n_evals)
    else:
        scores['Complexity'] = 0
...
def get_standard_engine_options(csv_options_str, engine_info):
    ...
    Only adds 'Hash' for Stockfish-like engines to avoid errors with other engines like Leela.
    """
    options = {}  # Start with an empty dictionary

    # Conditionally add the Hash option ONLY for Stockfish engines
    if 'stockfish' in engine_info['name'].lower():
        options["Hash"] = 256  # A reasonable default for analysis

    # Load any engine-specific options from the CSV file
    if csv_options_str:
        try:
            # These options can override the defaults (e.g., a different Hash size)
...
def universal_worker(args):
    ...
    MODIFIED to get evaluation for the played move in simulation mode.
    """
    (fen, engine_info, limit_params, worker_mode) = args

    try:
        # Use DEVNULL to hide engine's own stderr output
        with chess.engine.SimpleEngine.popen_uci(engine_info['path'], stderr=subprocess.DEVNULL) as engine:
            ...
            if worker_mode == 'oracle':
                # Get top N moves to establish ground truth
                ...
                # Now, get the specific evaluation for that move
...
def get_ground_truth_template(game, oracle_engine, session_data, pgn_path):
    ...
    # Analyze every 5th move after move 10 to get a good sample
    fens_to_analyze = [
...
def run_engine_simulations(ground_truth_template, engines, session_data, pgn_path):
    """Runs simulations to see which move each engine plays and gets its evaluation."""
    ...
    for engine_idx, engine_info in enumerate(engines):
        ...
        # Check if this engine's analysis is already complete
        is_complete = all(engine_name in sim_results.get(fen, {}) for fen in ground_truth_template.keys())
        if is_complete:
            logging.info(f"({engine_idx+1}/{total_engines}) Skipping {engine_name}, results already in session.")
            continue

        logging.info(f"--- ({engine_idx+1}/{total_engines}) Simulating with {engine_name} ---")
        limit = {'depth': MODEL_BUILD_DEPTH, 'time': MODEL_BUILD_TIMEOUT}
        ...
        # Robustly save after each engine completes
        session_data['in_progress_game']['simulation_results'] = dict(sim_results)
        save_session(session_data, pgn_path)
    
    return sim_results

def evaluate_frameworks(simulation_results, ground_truth_template, model_engines):
    """
    Calculates average scores for each engine under each framework and determines the best one.
    """
    logging.info("Calculating scores for all frameworks...")
    ...
    for fen, template in ground_truth_template.items():
        ...
        for engine_name, move_data in simulation_results[fen].items():
            ...
            # Calculate scores for this specific move
    ...
    # Average the scores for each engine
    ...
    # Now, build a model for each framework and get its R²
    framework_models = {}
    logging.info("\n--- Framework R² Comparison ---")
    for framework, scores_dict in avg_scores_by_framework.items():
        ...
        logging.info(f"  - {framework:<12}: R² = {r_squared:.4f}")
    
    if not framework_models:
        logging.error("Could not build any models.")
        return None, None

    best_framework_name = max(framework_models, key=lambda k: framework_models[k]['r_squared'])
...
def main():
    ...
    # Select a sparse but representative set of engines for model building
    ...
    try:
        with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
            game = chess.pgn.read_game(pgn_file) # Analyze first game with player
            while game:
    ...
    best_framework, final_model_bundle = evaluate_frameworks(sim_results, ground_truth, model_engines)
    if not best_framework:
        logging.error("Framework evaluation failed. Exiting."); return

    # Generate and save the final graph for the best framework
    ...
    logging.info(f"Analysis complete. Graph saved to {graph_path}")


if __name__ == "__main__":
    # This is necessary for compatibility with macOS and Windows
