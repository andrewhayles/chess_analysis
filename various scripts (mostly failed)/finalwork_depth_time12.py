...
NUM_SPARSE_ANALYSIS_POINTS = 10 # Number of engines for the fallback time-based analysis
...
def get_sparse_engines(full_engine_list, num_points, exclude_engines=None):
    """Selects a subset of engines with evenly spaced ratings, optionally excluding some."""
    ...
    if len(available_engines) <= num_points:
        return available_engines

    sorted_engines = sorted(available_engines, key=lambda x: x['rating'])
    ...
    for target in target_ratings:
        # Find the closest available engine that hasn't been picked yet
        available_indices = [i for i, _ in enumerate(sorted_engines) if i not in selected_indices]
...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                ...
                if best_overall_result:
                    ...
                    if chosen_strategy_name == 'time_first':
                        # If time-first wins, the final run uses the sparse set.
                        # If the checkpoint set is larger than the target, use the checkpoint set.
                        if len(checkpoint_engines) >= NUM_SPARSE_ANALYSIS_POINTS:
                            engines_for_final_run = checkpoint_engines
                        else:
                            engines_for_final_run = get_sparse_engines(model_engines, NUM_SPARSE_ANALYSIS_POINTS)
                    else: # depth-first won
                        # The final run uses the larger depth set.
                        engines_for_final_run = get_sparse_engines(model_engines, NUM_DEPTH_ANALYSIS_POINTS)
            ...
            final_engines_to_run = [e for e in engines_for_final_run if e not in checkpoint_engines]
