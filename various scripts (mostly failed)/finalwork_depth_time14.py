...
#   4. EFFICIENT FINAL ANALYSIS: Instead of re-running analysis, the script
#      keeps the checkpoint data and only analyzes the additional engines
#      needed to reach the target number of data points (30 for depth, 15 for time).
#   5. COMPREHENSIVE LOGGING: All checkpoint results and the final decision
#      are logged, providing a clear audit trail.
#   6. GUARANTEED ENGINE INCLUSION: The engine selection logic now ensures that
#      'maia_1100', 'maia_1200', and 'Leela_nodes_1000k' are always included
#      to maintain a wide and consistent rating spread for analysis.
#   7. NODE-BASED LIMITS: The script now correctly distinguishes between Leela
#      engines by parsing node counts from their names and applying a node-based
#      limit instead of a depth/time limit.
...
PRIMARY_MODEL_BUILD_TIMEOUT = 180 # (in seconds)

PRIMARY_PLAYER_ANALYSIS_DEPTH = 12
PRIMARY_PLAYER_ANALYSIS_TIMEOUT = 180 # (in seconds)
...
def get_standard_engine_options(csv_options_str, engine_info):
    ...
    # Explicitly set the SyzygyPath, even if empty, to disable default searching.
    # This will remove the "Failed to load" warnings.
...
def calculate_move_quality(centipawn_loss):
    ...
    # Ensure CPL is not negative
    cpl = max(0, centipawn_loss)

    # The maximum CPL we consider before the score becomes 0.
    # You can adjust this value. A higher value means a more forgiving scale.
    max_cpl_for_scoring = 200.0 

    if cpl >= max_cpl_for_scoring:
        return 0.0
    
    # Linearly decrease the score from 1.0 to 0.0 over the CPL range.
    quality = 1.0 - (cpl / max_cpl_for_scoring)
    
    return quality

# You might also consider a non-linear curve, like an exponential decay,
# for an even more nuanced model, which penalizes small mistakes less.
# For example:
# import math
# def calculate_move_quality_exponential(centipawn_loss):
#     if not isinstance(centipawn_loss, (int, float)) or centipawn_loss is None:
#         return 0.0
#     cpl = max(0, centipawn_loss)
#     # The 'k' factor determines how quickly the score drops.
#     # A smaller k means a slower drop. You can tune this.
#     k = 0.015 
#     return math.exp(-k * cpl)
...
def get_sparse_engines(full_engine_list, num_points, exclude_engines=None):
    ...
    # --- MODIFIED LOGIC TO GUARANTEE ENGINE INCLUSION ---

    # 1. Define required engines and find them in the full list
    ...
    # Log a warning if a required engine is not found in the CSV
    ...
    # 2. Prepare the list of available engines for the remaining selections
    ...
    # 3. Determine how many more engines to select
    num_to_select = num_points - len(required_engines)

    # If we don't need any more, or if required engines already exceed the target, return the required ones (trimmed if needed)
    if num_to_select <= 0:
        logging.info(f"The number of required engines ({len(required_engines)}) meets or exceeds the requested number of points ({num_points}). Returning only the required engines (or a subset).")
        return sorted(required_engines, key=lambda x: x['rating'])[:num_points]

    # If the remaining available engines are fewer than what we need, take them all
    if len(available_engines) <= num_to_select:
        final_list = required_engines + available_engines
        logging.info(f"Selected all {len(available_engines)} available engines plus {len(required_engines)} required engines.")
        return sorted(final_list, key=lambda x: x['rating'])

    # 4. Select the remaining engines using the original spacing logic
    sorted_available = sorted(available_engines, key=lambda x: x['rating'])
    ratings = np.array([eng['rating'] for eng in sorted_available])
    
    # Dynamically set the rating range based on the engines that are actually available
    ...
    # 5. Combine the required engines with the newly selected ones and return
    final_list = required_engines + additional_engines
    sparse_engine_list = sorted(final_list, key=lambda x: x['rating'])
    
    logging.info(f"Selected a sparse set of {len(sparse_engine_list)} engines for analysis, including {len(required_engines)} required engines.")
...
def estimate_rating_from_quality(model, quality_score):
    ...
    search_ratings = np.arange(1100, 3501, 1) # Widened range to match engine selection
...
def run_engine_simulations(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, is_checkpoint=False):
    ...
    if sim_results_key in session_data.get('in_progress_game', {}):
        sim_data = session_data['in_progress_game'][sim_results_key]
        first_fen = next(iter(ground_truth_template), None)
        if first_fen and first_fen in sim_data:
            processed_engine_names = set(sim_data[first_fen].keys())
            required_engine_names = {e['name'] for e in engines_to_process}
            if required_engine_names.issubset(processed_engine_names):
                logging.info(f"Found complete simulation results for '{analysis_params['name']}'. Skipping simulation.")
                return
    ...
    for fen in ground_truth_template.keys():
        for engine_info in engines_to_process:
            # Default limit based on the chosen strategy (depth or time)
            limit = {k: v for k, v in analysis_params.items() if k in ['depth', 'time'] and v is not None}

            # Check for node-specific engines
            if engine_info['name'].startswith('Leela_nodes_'):
                try:
                    # Parse the node count from the name (e.g., 'Leela_nodes_1k' -> 1000)
                    node_str = engine_info['name'].split('_')[-1]
                    if 'k' in node_str.lower():
                        nodes = int(float(node_str.lower().replace('k', '')) * 1000)
                    else:
                        nodes = int(node_str)
                    # Override the limit to be nodes-based
                    limit = {'nodes': nodes, 'time': analysis_params.get('time')}
                except (ValueError, IndexError):
                    logging.warning(f"Could not parse node count from engine name: {engine_info['name']}. Using default limits.")
...
def main():
    ...
    # Sort engines by rating to easily separate oracle and model engines
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            # Efficiently run the final analysis
            ...
            if final_engines_to_run:
                 ...
                 run_engine_simulations(ground_truth_template, final_engines_to_run, session_data, pgn_path, chosen_strategy_params)
            else:
