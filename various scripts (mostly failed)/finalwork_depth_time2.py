# chess_analyzer_multi_engine_v31_adaptive.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGE (from v30):
#   1. ADAPTIVE ANALYSIS: For each game, the script first runs the standard
#      depth-based analysis. If the resulting R-squared value is below a
#      threshold (0.75), it automatically performs a second, time-based
#      analysis and uses the dataset that produced the stronger correlation.
...
# --- FALLBACK (TIME-PRIORITY) ANALYSIS CONTROLS ---
FALLBACK_MODEL_BUILD_TIME = 6 # (in seconds)

# --- UNIVERSAL PLAYER & ORACLE CONTROLS ---
...
# --- ADAPTIVE THRESHOLD ---
R_SQUARED_FALLBACK_THRESHOLD = 0.75
...
def move_analysis_worker(args):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(engine_info['path']) as engine:
            ...
            limit = chess.engine.Limit(depth=depth, time=timeout) if depth is not None else chess.engine.Limit(time=timeout)
...
def get_averaged_ground_truth(game, oracle_engines, session_data, pgn_path):
    ...
    if not fens_to_analyze:
        logging.warning(f"No moves found for player {PLAYER_TO_ANALYZE} in this game."); return {}
...
def run_model_build_pass(ground_truth_evals, model_engines, depth, timeout, pass_name, session_data, pgn_path):
    logging.info(f"Building performance model ({pass_name} pass)...")
    
    # Use a unique key in the session to store results for this pass
    session_key = f"model_engine_results_{pass_name}"
    engine_qualities = session_data.get('in_progress_game', {}).get(session_key, {})

    for i, engine_info in enumerate(model_engines):
        if engine_info['name'] in engine_qualities: continue
        logging.info(f"({pass_name}) Processing engine {i+1}/{len(model_engines)}: {engine_info['name']}")
        tasks = [(fen, None, depth, timeout, engine_info, ground_truth_evals) for fen in ground_truth_evals]
        qualities_list = []
        print_progress_bar(0, len(tasks), prefix=f'Building ({engine_info["name"]} @ {pass_name}):', suffix='Complete', length=50)
        with multiprocessing.Pool(processes=NUM_ANALYSIS_CORES) as pool:
            for j, quality in enumerate(pool.imap_unordered(move_analysis_worker, tasks)):
                ...
                print_progress_bar(j + 1, len(tasks), prefix=f'Building ({engine_info["name"]} @ {pass_name}):', suffix='Complete', length=50)
        if qualities_list:
            engine_qualities[engine_info['name']] = np.mean(qualities_list)
            session_data['in_progress_game'][session_key] = engine_qualities
            save_session(session_data, pgn_path)
    ...
    return {
        ...
        "r_squared": models[best_model_name]['r2'],
        "ratings": ratings,
        "quality_scores": quality_scores,
        "model_type": best_model_name,
        "raw_model_results": engine_qualities
    }
...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data['games_to_process_indices']:
            ...
            # Primary (Depth-Based) Analysis Pass
            depth_results = run_model_build_pass(ground_truth_evals, model_engines, PRIMARY_MODEL_BUILD_DEPTH, PRIMARY_MODEL_BUILD_TIMEOUT, "Depth", session_data, pgn_path)
            
            final_results = depth_results
            analysis_method = "Depth-Priority"

            if depth_results and depth_results["r_squared"] < R_SQUARED_FALLBACK_THRESHOLD:
                logging.info(f"Depth-based R² ({depth_results['r_squared']:.4f}) is below threshold. Running time-based fallback...")
                
                # Fallback (Time-Based) Analysis Pass
                time_results = run_model_build_pass(ground_truth_evals, model_engines, None, FALLBACK_MODEL_BUILD_TIME, "Time", session_data, pgn_path)
                
                if time_results and time_results["r_squared"] > depth_results["r_squared"]:
                    logging.info(f"Time-based analysis yielded better R² ({time_results['r_squared']:.4f}). Using time-based model.")
                    final_results = time_results
                    analysis_method = "Time-Priority (Fallback)"
                else:
                    logging.info("Depth-based analysis remains the better model. Proceeding with depth-based results.")
            
            if not final_results:
                session_data['games_to_process_indices'].pop(0); save_session(session_data, pgn_path); continue

            model, r_squared, ratings, quality_scores, model_type, raw_model_results = final_results.values()
            ...
            plt.title(f"Game {game_num}: {white} vs. {black} ({analysis_method})"); plt.xlabel("Engine Elo Rating"); plt.ylabel("Move Quality"); plt.grid(True); plt.legend()
