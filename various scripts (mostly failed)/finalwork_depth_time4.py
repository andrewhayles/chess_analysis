...
def universal_worker(args):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(engine_info['path']) as engine:
            ...
            if worker_mode == 'oracle':
                ...
                for move_info in info:
                    if 'pv' in move_info and 'score' in move_info:
                        ...
                        template.append({'move': move.uci(), 'score': quality_score, 'raw_score': current_score})
                return template
            ...
            best_move_score = ground_truth_template[fen][0]['raw_score']
            try:
                # To get the score of the specific move played, we re-analyze with 'root_moves'
                info = engine.analyse(board, chess.engine.Limit(**limit_params), root_moves=[move_to_evaluate])
                if not info: return None
                played_move_score_white_pov = info[0]['score'].white().score(mate_score=30000)
                if played_move_score_white_pov is None: return None
                cpl = max(0, best_move_score - played_move_score_white_pov)
