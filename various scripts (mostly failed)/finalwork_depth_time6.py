...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                ...
                if depth_test_bundle and depth_test_bundle['r_squared'] < R_SQUARED_THRESHOLD:
                    ...
                    if time_test_bundle and time_test_bundle['r_squared'] > depth_test_bundle['r_squared']:
                        ...
                        decision_log = f"Switched to time-first strategy. Checkpoint R² ({time_test_bundle['r_squared']:.4f}) beat depth-first R² ({depth_test_bundle['r_squared']:.4f})."
                    else:
                        r2_depth = depth_test_bundle.get('r_squared', 0)
                        r2_time = time_test_bundle.get('r_squared', 0) if time_test_bundle else 0
                        decision_log = f"Kept depth-first strategy. Its checkpoint R² ({r2_depth:.4f}) was better than or equal to time-first's ({r2_time:.4f})."
            ...
            logging.info(f"--- Proceeding with full analysis using '{chosen_strategy_params['name']}' strategy. ---")
            
            best_results = run_full_analysis_for_game(game, ground_truth_template, model_engines, oracle_engines[0], chosen_strategy_params, session_data, pgn_path)
