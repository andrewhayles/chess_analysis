...
MODEL_SWITCH_CHECKPOINT = 50 # Number of engines to test before checking R² to switch strategy
NUM_SPARSE_ANALYSIS_POINTS = 60 # Number of engines for the fallback time-based analysis
...
def get_sparse_engines(full_engine_list, num_points):
    ...
    # Sort engines by rating to ensure predictable selection
    ...
    # Create the ideal, evenly spaced target ratings
    min_rating, max_rating = ratings[0], ratings[-1]
    target_ratings = np.linspace(min_rating, max_rating, num_points)
    
    # For each target rating, find the engine with the closest actual rating
    selected_indices = set()
    for target in target_ratings:
        closest_index = np.argmin(np.abs(ratings - target))
        selected_indices.add(closest_index)
        
    # Return the selected engines, preserving the original sort order by rating
    sparse_engine_list = [sorted_engines[i] for i in sorted(list(selected_indices))]
    
    logging.info(f"Selected a sparse set of {len(sparse_engine_list)} engines for time-based analysis.")
...
def build_model(ground_truth_template, model_engines, session_data, pgn_path, analysis_params, engine_subset_count=None):
    """Builds a regression model, optionally on a subset of engines for testing."""
    ...
    engines_to_process = model_engines[:engine_subset_count] if engine_subset_count is not None else model_engines
    
    for i, engine_info in enumerate(engines_to_process):
        ...
        prefix_str = f'Building ({engine_info["name"]} @ {analysis_params["name"]}):'
        if engine_subset_count:
            prefix_str = f'Checkpoint ({engine_info["name"]} @ {analysis_params["name"]}):'

        logging.info(f"Processing model engine {i+1}/{len(engines_to_process)}: {engine_info['name']}")
        tasks = [(fen, None, engine_info, limit, ground_truth_template, 'model') for fen in fens_for_model]
        scores_list = []
        print_progress_bar(0, len(tasks), prefix=prefix_str, suffix='Complete', length=50)
        with multiprocessing.Pool(processes=NUM_ANALYSIS_CORES) as pool:
            for j, score in enumerate(pool.imap_unordered(universal_worker, tasks)):
                if score is not None: scores_list.append(score)
                print_progress_bar(j + 1, len(tasks), prefix=prefix_str, suffix='Complete', length=50)
        
        if scores_list:
            engine_scores[engine_info['name']] = np.mean(scores_list)
            session_data.setdefault('in_progress_game', {}).setdefault(results_key, {})
...
def main():
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            engines_for_final_run = model_engines
            decision_log = ""
            
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                logging.info(f"Performing initial model check with {MODEL_SWITCH_CHECKPOINT} engines using depth-first strategy.")
                depth_test_bundle = build_model(ground_truth_template, model_engines, session_data, pgn_path, analysis_strategies['depth_first'], engine_subset_count=MODEL_SWITCH_CHECKPOINT)

                if depth_test_bundle and depth_test_bundle['r_squared'] < R_SQUARED_THRESHOLD:
                    logging.info(f"Checkpoint R² ({depth_test_bundle['r_squared']:.4f}) is below threshold. Comparing with time-first strategy.")
                    time_test_bundle = build_model(ground_truth_template, model_engines, session_data, pgn_path, analysis_strategies['time_first'], engine_subset_count=MODEL_SWITCH_CHECKPOINT)
                    
                    if time_test_bundle and time_test_bundle['r_squared'] > depth_test_bundle['r_squared']:
                        chosen_strategy_params = analysis_strategies['time_first']
                        engines_for_final_run = get_sparse_engines(model_engines, NUM_SPARSE_ANALYSIS_POINTS)
                        decision_log = f"Switched to sparse time-first strategy ({len(engines_for_final_run)} engines). Checkpoint R² ({time_test_bundle['r_squared']:.4f}) beat depth-first R² ({depth_test_bundle['r_squared']:.4f})."
                    else:
                        r2_depth = depth_test_bundle.get('r_squared', 0)
                        r2_time = time_test_bundle.get('r_squared', 0) if time_test_bundle else 0
                        decision_log = f"Kept depth-first strategy (full engine set). Its checkpoint R² ({r2_depth:.4f}) was better than or equal to time-first's ({r2_time:.4f})."
                elif depth_test_bundle:
                    decision_log = f"Kept depth-first strategy. Checkpoint R² ({depth_test_bundle['r_squared']:.4f}) met the threshold."
                else:
                    decision_log = "Could not build a checkpoint model. Defaulting to depth-first."
            else:
                decision_log = "Not enough model engines for a checkpoint test. Defaulting to depth-first."

            logging.info(decision_log)
            logging.info(f"--- Proceeding with full analysis using '{chosen_strategy_params['name']}' strategy on {len(engines_for_final_run)} engines. ---")
            
            best_results = run_full_analysis_for_game(game, ground_truth_template, engines_for_final_run, oracle_engines[0], chosen_strategy_params, session_data, pgn_path)
            
            if not best_results:
                logging.error("Final analysis run failed for the chosen strategy. Skipping game.")
                session_data['games_to_process_indices'].pop(0); save_session(session_data, pgn_path); continue
            # --- END ADAPTIVE LOGIC ---
