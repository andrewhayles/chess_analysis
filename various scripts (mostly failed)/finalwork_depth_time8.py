...
MODEL_SWITCH_CHECKPOINT = 10 # Number of engines to test before checking R² to switch strategy
...
def build_model(ground_truth_template, engines_to_process, session_data, pgn_path, analysis_params, is_checkpoint=False):
    """Builds a regression model on a given set of engines."""
    ...
    for i, engine_info in enumerate(engines_to_process):
        ...
        prefix_str = f'Checkpoint ({engine_info["name"]} @ {analysis_params["name"]}):' if is_checkpoint else f'Building ({engine_info["name"]} @ {analysis_params["name"]}):'
    ...
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        while session_data.get('games_to_process_indices'):
            ...
            if len(model_engines) >= MODEL_SWITCH_CHECKPOINT:
                ...
                logging.info(f"Performing initial model check with {len(checkpoint_engines)} engines using depth-first strategy.")
                depth_test_bundle = build_model(ground_truth_template, checkpoint_engines, session_data, pgn_path, analysis_strategies['depth_first'], is_checkpoint=True)

                if depth_test_bundle and depth_test_bundle['r_squared'] < R_SQUARED_THRESHOLD:
                    logging.info(f"Checkpoint R² ({depth_test_bundle['r_squared']:.4f}) is below threshold. Comparing with time-first strategy on the same {len(checkpoint_engines)} engines.")
                    time_test_bundle = build_model(ground_truth_template, checkpoint_engines, session_data, pgn_path, analysis_strategies['time_first'], is_checkpoint=True)
