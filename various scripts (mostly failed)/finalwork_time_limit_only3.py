# chess_analyzer_multi_engine_v22_single_game.py
# A script to analyze PGN chess games and estimate a specific player's rating.
#
# KEY CHANGE (from v21):
#   1. SINGLE GAME MODE: The script is now designed to process only the first
#      available game from the PGN file and then exit.
#   2. FIXED CORE USAGE: The number of CPU cores used for analysis has been
#      hard-coded to 2 for more controlled resource usage.
...
def get_eval_worker(args):
    ...
    try:
        with chess.engine.SimpleEngine.popen_uci(engine_info['path']) as engine:
            ...
            # Uses depth with a time limit as a safeguard
...
def get_averaged_ground_truth(game, oracle_engines, session_data, pgn_path):
    ...
    tasks = [(fen, engine_info, ORACLE_ANALYSIS_DEPTH, ORACLE_TIMEOUT) for fen in fens_to_analyze for engine_info in oracle_engines]
    total_tasks = len(tasks)
    position_evals = defaultdict(list)

    print_progress_bar(0, total_tasks, prefix='Ground Truth:', suffix='Complete', length=50)

    # Use the fixed number of cores
...
def main():
    ...
    # --- Main Game Loop ---
    with open(pgn_path, encoding="utf-8", errors="ignore") as pgn_file:
        if session_data['games_to_process_indices']:
            ...
            logging.info("All games in PGN have been analyzed.")
