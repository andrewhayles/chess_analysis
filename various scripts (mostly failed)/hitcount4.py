...
def process_item(task_params):
    ...
    """
    board_fen, oracle_moves_uci, engine_info, game_id, move_number, run_name, weights, log_queue = task_params
    ...
    try:
        ...
        if not isinstance(test_engine_path, str) or not test_engine_path: return
        ...
        score_for_move = 0.0
        for i in range(len(oracle_moves_uci)):
            if test_result.move.uci() == oracle_moves_uci[i]:
...
def main_tuner():
    ...
    if os.path.exists(ORACLE_CACHE_PATH):
        with open(ORACLE_CACHE_PATH, 'r') as f:
            try:
                ...
                print(f"Warning: Oracle cache file at '{ORACLE_CACHE_PATH}' is corrupted or empty. Starting fresh.")
                oracle_cache = {}
        print(f"Loaded {len(oracle_cache)} positions from oracle cache.")

    positions_needing_oracle_analysis = [p for p in positions_to_analyze if p['fen'] not in oracle_cache]
    if positions_needing_oracle_analysis:
        print(f"Found {len(positions_needing_oracle_analysis)} new positions needing oracle analysis.")
        oracle_engine = open_engine(oracle_engine_path)
        oracle_engine.configure({"Threads": ENGINE_THREADS})
        for pos in tqdm(positions_needing_oracle_analysis, desc="Analyzing new positions with Oracle"):
            board = chess.Board(pos['fen'])
            max_moves_needed = max(p['num_moves'] for p in PARAMETER_SETS_TO_TEST)
            limit = chess.engine.Limit(depth=ORACLE_ANALYSIS_DEPTH, time=ORACLE_ANALYSIS_TIMEOUT)
            analysis = oracle_engine.analyse(board, limit, multipv=max_moves_needed)
            oracle_cache[pos['fen']] = [info['pv'][0].uci() for info in analysis]
        oracle_engine.quit()
        with open(ORACLE_CACHE_PATH, 'w') as f:
            json.dump(oracle_cache, f)
        print("Oracle cache updated and saved.")
    else:
        print("All positions found in oracle cache. No new oracle analysis needed.")
    ...
    if os.path.exists(DETAILED_LOG_PATH):
        ...
        try:
            ...
            print("Log file is empty. Starting fresh analysis.")
            pass
    ...
    for param_set in PARAMETER_SETS_TO_TEST:
        ...
        tasks = []
        # *** FIX: Use a set to prevent duplicate tasks from being created for a single run ***
        task_ids_for_this_run = set()
        for pos in positions_to_analyze:
            oracle_moves = oracle_cache.get(pos['fen'], [])[:num_moves]
            for engine_info in all_engines_info:
                item_id = (run_name, pos['game_id'], pos['move_number'], engine_info['engine_name'])
                
                if item_id not in completed_items and item_id not in task_ids_for_this_run:
                    tasks.append((pos['fen'], oracle_moves, engine_info, pos['game_id'], pos['move_number'], run_name, weights, log_queue))
                    task_ids_for_this_run.add(item_id)
        
        if not tasks:
            print(f"\nAll items for run '{run_name}' are already complete. Skipping.")
            continue
        
        print(f"\nAnalyzing {len(tasks)} new items for '{run_name}'...")
    ...
    if os.path.exists(DETAILED_LOG_PATH):
        try:
            ...
            print("Could not generate summary because the log file is empty.")
